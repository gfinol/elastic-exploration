...
import crucial.execution.ibm.IBMCloudFunctionsExecutorService;
 ...
 *
 */
public class TMServerlessUTSIBM {
    ...
    public static long run(final int parallelism, final int numberOfIterationsPerWave, final int depth) {
        ...
        TMServerlessUTSIBM waves = new TMServerlessUTSIBM(parallelism, numberOfIterationsPerWave);
        waves.run(depth);

        return waves.counter.get();
    }

    private TMServerlessUTSIBM(final int parallelism, final int numberOfIterationsPerWave) {
        ...
        while (activeThreads.get() > 0 || !queue.isEmpty()) {
            ...
            if (bag != null) {
                ...
                if (random.nextDouble() < 0.1) {
                    System.out.println(currentActiveThreads + " pending threads // actually " + queue.size()
                            + " bags in the queue");
                }

                // Trick
                /*
                 * if (step==0 && currentActiveThreads > 800) { step++; parallelism = 5;
                 * numberOfIterationsPerWave = 2_500_000;
                 * System.out.println("******* APPLIED STEP 0"); } if (step==1 &&
                 * currentActiveThreads > 1300) { step++; parallelism = 5;
                 * numberOfIterationsPerWave = 5_000_000;
                 * System.out.println("******* APPLIED STEP 1"); } if (step==2 &&
                 * currentActiveThreads < 1100) { step++; parallelism = 5;
                 * numberOfIterationsPerWave = 2_500_000;
                 * System.out.println("******* APPLIED STEP 2"); } if (step==3 &&
                 * currentActiveThreads < 100) { step++; parallelism = 5;
                 ...
                 * System.out.println("Requeueing bag. Current active threads = " +
                 * currentActiveThreads); queue.offer(bag); } else {
                 */
                ...
                if (resultBag != null) {
                    ...
                    /*
                     * if (currentActiveThreads > 1000) { parallelismParam = 2; }
                     */
    ...
    public static void main(String[] args) {
        ...
        try {
            executorService = new IBMCloudFunctionsExecutorService();
