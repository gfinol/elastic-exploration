...
public class CmdLineOptions {
    ...
    public int g = 8; // vertexs to process at each task
    ...
    public CmdLineOptions(final String[] args) {
        ...
        for (int curArg = 0; curArg < args.length; curArg++) {
            ...
            if (arg.equalsIgnoreCase("-seed") || arg.equalsIgnoreCase("-s")) {
                ...
                final String arg2 = args[curArg];
                try {
                    parallelism = Integer.parseInt(arg2);
                } catch (final Exception e) {
                    throw new IllegalArgumentException("workers argument is not parseable as an integer " + arg2);
                }
            } else if (arg.equalsIgnoreCase("-group") || arg.equalsIgnoreCase("-g")) {
                curArg++;
                if (curArg >= args.length) {
                    throw new IllegalArgumentException(
                            "Illegal " + arg + " argument with no corresponding place:timespan argument");
                ...
                final String arg2 = args[curArg];
                try {
                    g = Integer.parseInt(arg2);
                } catch (final Exception e) {
                    throw new IllegalArgumentException("g argument is not parseable as an integer " + arg2);
                }
    ...
    static void printUsage() {
        ...
        System.err.println("-w <INT>\t\tSet the workers (parallelism) to be used");
    }
